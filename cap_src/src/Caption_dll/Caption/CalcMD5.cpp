...
static void MD5Encode(unsigned char *, const DWORD *, unsigned int);
static void MD5Decode(DWORD *, const unsigned char *, unsigned int);
...
static void MD5Encode(unsigned char *output, const DWORD *input, unsigned int len)
{
  unsigned int i, j;
...
static void MD5Decode(DWORD *output, const unsigned char *input, unsigned int len)
{
  unsigned int i, j;
...
BOOL CalcMD5FromDRCSPattern(BYTE *pbHash, char *pcHashStr, const BYTE *pbPatternData, const DWORD dwDataLen)
{
    MD5_CTX ctx;
    MD5Init(&ctx);
    MD5Update(&ctx, pbPatternData, dwDataLen);
    MD5Final(pbHash, &ctx);

    char MD5tmp[10];
    for (int i = 0; i < 16; i++) {
        sprintf(MD5tmp, "%02X", pbHash[i]);
