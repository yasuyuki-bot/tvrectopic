...
protected:
    ...
    void set_name(const TCHAR *name, const TCHAR *ext)
    {
    ...
    int open(const TCHAR *file_type)
    {
        if (this->fp)
            return -2;
...
void CLogHandler::SetName(void)
{
    ...
    const TCHAR *name = cp->LogFileName;
	...
	if(!this->aligncenter) {//position set by SetPos
		for (int i = 0; it != capList.end(); it++, i++) {
			...
			if (((*it)->outCharSizeMode != STR_SMALL) && ((*it)->outHLC == HLC_box)) {
			    ...
			    fprintf(fp, "Dialogue: 0,%01d:%02d:%02d.%02d,%01d:%02d:%02d.%02d,Box,,0000,0000,0000,,{\\pos(%d,%d)\\fscx%d\\fscy%d\\3c&H%02x%02x%02x&}",
			            sH, sM, sS, sMs, eH, eM, eS, eMs, iBoxPosX, iBoxPosY, iBoxScaleX, iBoxScaleY, (*it2)->outCharColor.ucB, (*it2)->outCharColor.ucG, (*it2)->outCharColor.ucR);
			...
			if (((*it)->outCharSizeMode != STR_SMALL) && ((*it)->outHLC == HLC_draw)) {
			    ...
			    int iBoxScaleY = 100;   //*((*it)->outCharH + (*it)->outCharVInterval) / (*it)->outCharH;
			    fprintf(fp, "Dialogue: 0,%01d:%02d:%02d.%02d,%01d:%02d:%02d.%02d,Box,,0000,0000,0000,,{\\pos(%d,%d)\\3c&H%02x%02x%02x&\\p1}m 0 0 l %d 0 %d %d 0 %d{\\p0}\r\n",
			            sH, sM, sS, sMs, eH, eM, eS, eMs, iBoxPosX, iBoxPosY, (*it2)->outCharColor.ucB, (*it2)->outCharColor.ucG, (*it2)->outCharColor.ucR, iBoxScaleX, iBoxScaleX, iBoxScaleY, iBoxScaleY);
			}
			...
			if ((*it2)->outCharColor.ucR != 0xff || (*it2)->outCharColor.ucG != 0xff || (*it2)->outCharColor.ucB != 0xff)
				fprintf(fp, "\\c&H%02x%02x%02x&", (*it2)->outCharColor.ucB, (*it2)->outCharColor.ucG, (*it2)->outCharColor.ucR);
			if ((*it2)->outUnderLine)
				fprintf(fp, "\\u1");
			if ((*it2)->outBold)
				fprintf(fp, "\\b1");
...
int _tmain(int argc, _TCHAR *argv[])
{
    ...
    CCaption2AssParameter *param = NULL;
    pid_information_t *pi = NULL;
    cli_parameter_t   *cp = NULL;
    ass_setting_t     *as = NULL;
    ...
    param = static_cast<CCaption2AssParameter *>(app.GetParam(C2A_PARAM_ALL));
    pi = static_cast<pid_information_t *>(app.GetParam(C2A_PARAM_PID));
    cp = static_cast<cli_parameter_t *>(app.GetParam(C2A_PARAM_CLI));
    as = static_cast<ass_setting_t *>(app.GetParam(C2A_PARAM_ASS));
